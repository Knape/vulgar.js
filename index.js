...
module.exports.toFraction = (string) => {
  ...
  for (let n = 0; n < stringLength; n++) {
    ...
    s = 'u' + s;
    if (vulgar[s]) {
      let needsSpace = string.charAt(n - 1) === ' ' || n === 0 ? '' : ' ';
...
module.exports.toDecimal = (fraction) => {
  if (fraction === null || typeof fraction === 'undefined') throw 'Please supply a fractal';
  return fraction.replace(/((\d*)\/(\d*))/g, (_, f) => convertFraction(f));
}
